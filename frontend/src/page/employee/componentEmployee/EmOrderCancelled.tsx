...
function EmOrderCancelled() {
    ...
    const fetchPendingOrders = async () => {
        try {
            const response = await axios.get(`${API_URL}/api/food/getCancelledByCustomerOrEmployee/${id}`);
                ...
                <Box key={item._id} display="flex" gap={2} alignItems="center" sx={{ marginBottom: 20 }}>
                    ...
                    >
                        {loading ? (
                            ...
                            isPaymentPending ? "ขอบคุณที่ใช้บริการ" :  "ยกเลิก"
                        )}
                    </Button>
...
export default EmOrderCancelled;