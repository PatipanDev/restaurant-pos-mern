...
exports.addFood = async (req, res) => {
  ...
  try {
    ...
    let food = await Food.findOne({ food_Name, product_Category });
    if (food) {
    ...
    res.status(201).json({
      ...
      product_Category: food.product_Category,
...
exports.getFoods = async (req, res) => {
  try {
    const foods = await Food.find().populate(['product_Category', 'chef_Id', 'owner_Id']);
